...
# --- Access gate ---
# Protects the deployed app from unrelated visitors burning through the
# owner's Gemini/Serper API quota. Set ACCESS_CODE in .env (or as a
# Streamlit Cloud secret when deployed) -- if it's left blank, the gate
# is skipped entirely (useful for local-only testing).
...
if ACCESS_CODE:
    ...
    if not st.session_state["access_granted"]:
        st.info("This app is access-restricted. Enter the access code to continue.")
...
st.sidebar.info(
    ("MFR URL / spec-sheet fields will be filled using verified web "
     "lookups (Serper search key detected)." if SEARCH_API_KEY else
     "MFR URL / spec-sheet fields stay blank -- no search API key "
     "configured yet, so those fields are honestly left empty rather "
     "than guessed. See enrich.py docstring for details.")
)
...
if input_mode == "Upload file":
    ...
    pasted_text = st.text_area(
        ...
        height=200,
        placeholder=(
            "e.g. Item: DCB518ASTS06G, Diablo brand 1/2 in x 18 in sanding "
            "belt, pack of 6, distributed by Freud Inc (code 2435)."
        ),
    )
